...
func (a *API) handlerRawProjectsCreate(w http.ResponseWriter, r *http.Request) {
	...
	rawProject, rawStacks, err := a.ProfileService.CreateRawProject(r.Context(), userID, services.CreateRawProjectParams{
		Label:       *params.Label,
	...
	if err != nil {
		...
		respond.WithError(w, http.StatusInternalServerError, "Couldn't create rawProject", err)
		return
	}
